...
##### Roles
An abstract base `Role` model has been added. The platform's existing `OrganizationRole` and `ProjectRole` models are retained but now implement `Role` and define a  `FK` relation to `auth.Group`. Each `Group` has an associated set of `Permissions`. An `OrganiztionRole` for a user with an organization *admin role* will be associated to the `OrgAdmin` `Group` and will have a corresponding set of permissions. Two further class-based roles have been defined for *anonymous* and *public* users respectively.
